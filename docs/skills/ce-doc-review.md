...
`ce-doc-review` is the **document review** skill — sibling to `/ce-code-review` for the docs side of the chain. It analyzes a requirements-only unified plan, implementation-ready plan, or legacy document, selects reviewer personas based on what the doc contains (design surfaces, high-stakes domains), dispatches them in parallel, then auto-applies safe markdown fixes and routes the rest through a structured four-option interaction (per-finding walk-through, auto-resolve with best judgment, append to Open Questions, report-only).
...
- **Interleaved concerns** — design, feasibility, and high-stakes domain risks all need different lenses, but a single reviewer prioritizes one
...
- **Always-on personas** for coherence, feasibility, and whole-doc
- **Conditional personas** selected based on doc content — design-lens, adversarial
...
- **`/ce-brainstorm` Phase 4** — offered as one of the post-doc options ("Agent review of Product Contract"); runs interactive with full scrutiny, since validating the plan is exactly what brainstorm exists for
...
`ce-code-review` reviews diffs (code changes); `ce-doc-review` reviews docs (requirements, plans). Different reviewer personas, different findings shape, different routing. Both share the multi-persona dispatch + synthesis pattern.
