...
**Jira-ticket prefixing (stacked on conventional).** Resolve a Jira ticket ID before composing the subject, in priority order: (1) current branch name — match `^.*/([A-Z][A-Z0-9_]+-\d+[A-Za-z0-9#_-]*)$` against `git branch --show-current`, then strip any trailing `#` or `-<alphanumeric-segment>` to get the **bare** ID; (2) most recent commit subject match `^([A-Z][A-Z0-9_]+-\d+)\b` from `git log --oneline -10` (commit subjects already carry the bare ID, so no stripping needed); (3) empty. The two org-specific suffix conventions — `HVD-9554#` (CI/CD pipeline routing) and `HVD-9554-2` / `HVD-9554-hotfix` (revision/hotfix for the same ticket) — are both normalized to the bare `HVD-9554` for commit-subject prefixing. The suffix is preserved on the branch name and never appears in commit subjects. **Do not ask the user for a ticket ID** — `ce-commit` is the commit-only sibling; ticket capture happens upstream in `ce-brainstorm` / `ce-plan` / `ce-ideate`'s Phase 0.0a, and `ce-commit` only inherits. When a bare ticket ID resolves, the subject shape is `<bare-TICKET-ID> <type>(<scope>): <subject>` (e.g. `HVD-9554 feat(ui): add borders to word boxes`). When no ticket resolves, the subject is the standard conventional-commits shape with no prefix token. The body is unaffected either way.
...
If the current branch from the context above is `main`, `master`, or the resolved default branch from Step 1, automatically create a feature branch before committing. Derive the branch name from the change content, create it with `git checkout -b <branch-name>`, run `git branch --show-current` to confirm, and use the new branch as the current branch for the rest of the workflow. Do not ask whether to branch — committing on the default branch is not an option here. **If a Jira ticket ID resolved in Step 2, the branch name is `<pr-prefix>/<TICKET-or-TICKET-with-suffix>` instead of a content-derived name.** Resolve `<pr-prefix>` in priority order: (1) `GITHUB_PR_PREFIX_USERNAME` config key in `.compound-engineering/config.local.yaml` (preferred — env var also works as a per-session override); (2) `git for-each-ref --format='%(refname:short)' refs/heads/` inference + blocking-question confirm; (3) ask once. The suffix is preserved on the branch name (CI/CD routing); the bare ID prefixes the commit subject. When no ticket resolved, keep the existing content-derived name.
