...
If the current branch from the context above is `main`, `master`, or the resolved default branch from Step 1, automatically create a feature branch before committing. Derive the branch name from the change content, create it with `git checkout -b <branch-name>`, run `git branch --show-current` to confirm, and use the new branch as the current branch for the rest of the workflow. Do not ask whether to branch — committing on the default branch is not an option here. **If a Jira ticket ID resolved in Step 2, the branch name is `<pr-prefix>/<TICKET-or-TICKET-with-suffix>` instead of a content-derived name.** Resolve `<pr-prefix>` in priority order: (1) `GITHUB_PR_PREFIX_USERNAME` env var (preferred — exported in `~/.zshrc` or `~/.bashrc` by `/ce-setup` Phase 3); (2) `git for-each-ref --format='%(refname:short)' refs/heads/` inference + blocking-question confirm; (3) ask once. The suffix is preserved on the branch name (CI/CD routing); the bare ID prefixes the commit subject. When no ticket resolved, keep the existing content-derived name.
