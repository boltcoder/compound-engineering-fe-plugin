...
describe("ce-plan output:html mode", () => {
  ...
  test("post-generation menu offers format-keyed option 4 (browser for html)", () => {
    // Under exclusive output mode, the plan is exactly one artifact — either
    // .md or .html. The menu's option 4 is format-keyed: browser for html.
    // The legacy mutual-exclusion gate with sibling-rerender logic is gone.
